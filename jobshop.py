...
class Problem:
    ...
    def optimize(self):
        ...
        for node in self.V:
            self._PMP[node] = []
            self._SMP[node] = []
            if node in self._PJ.keys():
                pass
            elif node in [0]+cumsum(self.J)[:-1]:
                self._PJ[node] = [-1]
            else:
                self._PJ[node] = [node-1]
    ...
    def update_r(self, node):
        ...
        for i in tsort:
            ...
            try:
                m2 = max(self.getr(x)+self.getD(x) for x in self.PMP(i))
            except:
                m2 = 0
    ...
    def update_t(self, node):
        ...
        for i in tsort:
            ...
            try:
                m2 = max(self.gett(x)+self.getD(x) for x in self.SMP(i))
            except:
                m2 = 0
